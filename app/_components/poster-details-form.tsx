...
type MobileSectionId = "location" | "map" | "output";
...
const MOBILE_SECTION_DEFAULT_STATE: Record<MobileSectionId, boolean> = {
  location: true,
  map: false,
  output: false
};
...
export function PosterDetailsForm() {
  ...
  const [mobileSectionState, setMobileSectionState] = useState<Record<MobileSectionId, boolean>>(
    MOBILE_SECTION_DEFAULT_STATE
  );
  ...
  const hasTheme = form.theme.length > 0;
  const hasCoordinatePair = latitudeValue.length > 0 && longitudeValue.length > 0;
  const locationIsReady = isCityCenterMode ? hasCity && hasCountry : hasCoordinatePair;
  const outputIsReady = hasTheme;
  const mapStatusLabel = "Optional";
  const mapStatusTone = "is-optional";
  ...
  if (isCityCenterMode) {
    ...
    missingRequirements.push("enter both latitude and longitude");
  }
  ...
  useEffect(() => {
    ...
    const loadThemes = async () => {
      try {
        ...
        const requestedTheme = data.themes.find((theme) => theme.id === selectedThemeFromQuery)?.id;
        const fallbackTheme = data.themes[0]?.id ?? "";
  ...
  useEffect(() => {
    ...
    const syncMobileState = () => setIsMobileAccordion(mediaQuery.matches);
  ...
  const isSectionOpen = (sectionId: MobileSectionId) => {
    if (!isMobileAccordion) {
      return true;
    }

    return mobileSectionState[sectionId];
  };

  const onToggleMobileSection = (sectionId: MobileSectionId) => {
    if (!isMobileAccordion) {
      return;
    }

    setMobileSectionState((previous) => {
      const nextState: Record<MobileSectionId, boolean> = {
        location: false,
        map: false,
        output: false
      };
      nextState[sectionId] = !previous[sectionId];
      return nextState;
    });
  };

  const onJumpToSection = (sectionId: MobileSectionId) => {
    if (isMobileAccordion) {
      setMobileSectionState({
        location: sectionId === "location",
        map: sectionId === "map",
        output: sectionId === "output"
      });
    }

    if (typeof document !== "undefined") {
      document.getElementById(`details-${sectionId}-title`)?.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  };
  ...
  const onUseCurrentLocation = () => {
    ...
    if (isCityCenterMode) {
      setGeolocationStatus({
        tone: "info",
        message: "Switch to coordinates mode to use your current location."
      });
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setGeolocationStatus({
          tone: "success",
          message: "Current coordinates added. Center mode switched to coordinates."
        });
        setIsLocating(false);
      },
      (geoError) => {
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!isCityCenterMode && !hasCoordinatePair) {
      setError("Latitude and longitude are required when using coordinate center mode.");
      return;
    }
  ...
  return (
    <section className="details-shell" aria-label="Poster details step">
      <header className="section-header">
        ...
        <h1>Set location and export details</h1>
        <p>Start with required details first, then adjust optional map and output settings.</p>
      </header>

      <form className="details-form" onSubmit={onSubmit}>
        <section className="details-priority-panel" aria-label="Required fields checklist">
          <p className="details-priority-title">What do you want to edit first?</p>
          <div className="details-priority-actions">
            <button type="button" className="ghost-button" onClick={() => onJumpToSection("location")}>
              Start with location
            </button>
            <button type="button" className="ghost-button" onClick={() => onJumpToSection("output")}>
              Update theme and export
            </button>
            <button type="button" className="ghost-button" onClick={() => onJumpToSection("map")}>
              Fine-tune map style
            </button>
          </div>
        </section>
        ...
        <section
          className={`details-section details-accordion-section${isSectionOpen("location") ? " is-expanded" : " is-collapsed"}`}
          aria-labelledby="details-location-title"
        >
          <div className="details-section-header">
            <h2 id="details-location-title">
              <button
                type="button"
                className="details-accordion-trigger"
                onClick={() => onToggleMobileSection("location")}
                aria-expanded={isSectionOpen("location")}
                aria-controls="details-location-content"
              >
                <span className="details-trigger-content">
                  <span>Start here: location details</span>
                  <span className={`details-status-pill ${locationIsReady ? "is-complete" : "is-required"}`}>
                    {locationIsReady ? "Required complete" : "Required"}
                  </span>
                </span>
            ...
            <p>Choose one center method: city/country or exact coordinates.</p>
          </div>

          <div
            id="details-location-content"
            className={`details-accordion-content${isSectionOpen("location") ? " is-open" : ""}`}
          >
            <div className="details-grid">
              ...
              {isGeolocationSelected ? (
                <p className="details-grid-full helper-note">
                  Geolocation is active. Coordinates mode is selected.
                </p>
              ) : null}
            ...
            {isMobileAccordion ? (
              <div className="details-section-nav">
                <button type="button" className="ghost-button" onClick={() => onJumpToSection("output")}>
                  Next: theme and export
                </button>
              </div>
            ) : null}
          </div>
        ...
        <section
          className={`details-section details-accordion-section${isSectionOpen("map") ? " is-expanded" : " is-collapsed"}`}
          aria-labelledby="details-map-title"
        >
          <div className="details-section-header">
            <h2 id="details-map-title">
              <button
                type="button"
                className="details-accordion-trigger"
                onClick={() => onToggleMobileSection("map")}
                aria-expanded={isSectionOpen("map")}
                aria-controls="details-map-content"
              >
                <span className="details-trigger-content">
                  <span>Fine-tune map coverage and marker</span>
                  <span className={`details-status-pill ${mapStatusTone}`}>{mapStatusLabel}</span>
                </span>
                <span className="details-accordion-indicator" aria-hidden="true">
                  {isSectionOpen("map") ? "-" : "+"}
                </span>
            ...
            <p>Adjust map distance and marker style. Your selected center mode is kept above.</p>
          </div>

          <div
            id="details-map-content"
            className={`details-accordion-content${isSectionOpen("map") ? " is-open" : ""}`}
          >
            <div className="details-grid">
              <label className="details-grid-full">
                ...
                <input
                  ...
                  min={100}
                  onChange={(event) =>
                    setForm((previous) => ({ ...previous, distance: Number(event.target.value) || 100 }))
                  }
                />
              </label>
        ...
        <section
          className={`details-section details-accordion-section${isSectionOpen("output") ? " is-expanded" : " is-collapsed"}`}
          aria-labelledby="details-output-title"
        >
          <div className="details-section-header">
            <h2 id="details-output-title">
              <button
                type="button"
                className="details-accordion-trigger"
                onClick={() => onToggleMobileSection("output")}
                aria-expanded={isSectionOpen("output")}
                aria-controls="details-output-content"
              >
                <span className="details-trigger-content">
                  <span>Finish: theme and export output</span>
                  <span className={`details-status-pill ${outputIsReady ? "is-complete" : "is-required"}`}>
                    {outputIsReady ? "Theme selected" : "Theme required"}
                  </span>
                </span>
                <span className="details-accordion-indicator" aria-hidden="true">
                  {isSectionOpen("output") ? "-" : "+"}
                </span>
              </button>
            </h2>
            <p>Review theme, file format, and size. Defaults are pre-filled, so change only what you need.</p>
          </div>

          <div
            id="details-output-content"
            className={`details-accordion-content${isSectionOpen("output") ? " is-open" : ""}`}
          >
            <div className="details-grid">
              <label className="details-grid-full">
                Theme
                <select value={form.theme} onChange={(event) => onThemeChange(event.target.value)} required>
                  {themes.map((theme) => (
