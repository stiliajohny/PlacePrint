  ...
  return (
    <section className="theme-showcase" aria-label="Theme gallery">
      <header className="showcase-header">
        ...
        <h1>Choose a theme from real map previews</h1>
        <p>
          Each theme card below is tied to one city from your list and generated through the same API pipeline used for
          final poster output.
        </p>
      </header>
      ...
      <div className="theme-grid">
        {items.map((item) => (
          <article key={item.themeId} className="theme-card">
            {item.previewUrl ? (
            ...
            <div className="theme-card-body">
              ...
              <Link className="theme-select-link" href={`/details?theme=${encodeURIComponent(item.themeId)}`}>
                Select Theme
              </Link>
            </div>
          </article>
