...
function markerBaseRadius(scaleFactor: number): number {
  // Keep marker legible on tiny exports while preserving relative size differences.
  return clamp(15 * scaleFactor, 4.5, 28);
}
...
function resolveMarkerGeometry(scene: PosterScene): MarkerGeometry | null {
  ...
  const outerRadius = Math.max(
    3.6,
    markerBaseRadius(scene.scaleFactor) * markerSizeMultiplier(scene.markerSize)
  );
