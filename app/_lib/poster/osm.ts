...
function orderedOverpassEndpoints(endpoints: string[]): string[] {
  if (!preferredOverpassEndpoint || !endpoints.includes(preferredOverpassEndpoint)) {
    return endpoints;
  }

  return [
    preferredOverpassEndpoint,
    ...endpoints.filter((endpoint) => endpoint !== preferredOverpassEndpoint)
  ];
}
...
async function fetchOverpassUncached(query: string, cacheKey: string): Promise<OverpassElement[]> {
  const maxAttempts = Math.max(1, OVERPASS_MAX_RETRIES);
  const totalTimeoutMs = Math.max(1_000, OVERPASS_TOTAL_TIMEOUT_MS);
  const deadlineAtMs = Date.now() + totalTimeoutMs;
  let lastError: Error | undefined;

  attempts: for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const endpoints = orderedOverpassEndpoints(OVERPASS_API_URLS);
    for (const endpoint of endpoints) {
      const remainingBeforeRateLimitMs = deadlineAtMs - Date.now();
      if (remainingBeforeRateLimitMs <= 0) {
        lastError = new Error(`Overpass total timeout reached (${totalTimeoutMs}ms).`);
        logger.warn("Overpass total timeout reached before request", {
          cacheKey,
          attempt,
          maxAttempts,
          totalTimeoutMs
      ...
      const remainingBeforeRequestMs = deadlineAtMs - Date.now();
      if (remainingBeforeRequestMs <= 0) {
        lastError = new Error(`Overpass total timeout reached (${totalTimeoutMs}ms).`);
        logger.warn("Overpass total timeout reached after rate limit wait", {
          cacheKey,
          attempt,
          maxAttempts,
          totalTimeoutMs
      ...
      const requestTimeoutMs = Math.max(750, Math.min(OVERPASS_REQUEST_TIMEOUT_MS, remainingBeforeRequestMs));
      ...
      logger.debug("Overpass request attempt", {
        ...
        requestTimeoutMs,
        remainingBudgetMs: remainingBeforeRequestMs
      });
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (preferredOverpassEndpoint === endpoint) {
            preferredOverpassEndpoint = undefined;
          }
          continue;
        }
    ...
    if (attempt < maxAttempts) {
      const remainingBeforeBackoffMs = deadlineAtMs - Date.now();
      if (remainingBeforeBackoffMs <= 0) {
        lastError = new Error(`Overpass total timeout reached (${totalTimeoutMs}ms).`);
        logger.warn("Overpass total timeout reached before retry backoff", {
          cacheKey,
          attempt,
          maxAttempts,
          totalTimeoutMs
      ...
      const delayMs = Math.min(retryDelay(attempt), remainingBeforeBackoffMs);
...
async function fetchOverpass(query: string, cacheKey: string): Promise<OverpassElement[]> {
  ...
  const inFlight = overpassInFlight.get(cacheKey);
  if (inFlight) {
    logger.debug("Overpass in-flight request reused", { cacheKey });
    return inFlight;
  }

  const request = fetchOverpassUncached(query, cacheKey).finally(() => {
    overpassInFlight.delete(cacheKey);
  });
