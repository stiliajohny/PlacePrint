...
### Added
- **uv package manager support** ([PR #20](../../pull/20))
...
- **Python version specification** - `requires-python = ">=3.11"` in pyproject.toml (fixes [#79](../../issues/79))
- **Coordinate override** - `--latitude` and `--longitude` arguments to override the geocoded center point (existing from upstream PR #106, clarifies [#100](../../issues/100))
  - Still requires `--city` and `--country` for display name
  - Useful for precise location control

### Fixed
- **Z-order bug** - Roads now render above parks and water features (fixes [#39](../../issues/39), relates to [PR #42](../../pull/42))
...
- **Text scaling for landscape orientations** - Font size now scales based on `min(height, width)` instead of just width (fixes [#112](../../issues/112))
...
### Added
- **Custom coordinates support** - `--latitude` and `--longitude` arguments ([#106](../../pull/106))
- **Emerald theme** - Lush dark green aesthetic with mint accents ([#114](../../pull/114))
- **GitHub Actions** - PR checks workflow ([#98](../../pull/98))
- **Conflict labeling** - Auto-label PRs with merge conflicts

### Changed
- **Default theme** changed from `feature_based` to `terracotta` ([#131](../../pull/131))
- **Default distance** changed from 12000m to 18000m ([#128](../../pull/128))
- **Max dimensions** enforced at 20 inches for width/height (supports up to 4K resolution) ([#128](../../pull/128), [#129](../../pull/129))

### Removed
- `feature_based` theme ([#131](../../pull/131))

### Fixed
- Cache directory handling ([#109](../../pull/109))
...
### Added
- **SVG/PDF export** - `--format` flag for vector output ([#57](../../pull/57))
- **Variable poster dimensions** - `-W` and `-H` arguments ([#59](../../pull/59))
- **Caching** - Downloaded OSM data is now cached locally
- **Rate limiting** - 0.3s delay between API requests

### Fixed
- Map warping issues with variable dimensions ([#59](../../pull/59))
- Edge nodes retention for complete road networks ([#27](../../pull/27))
...
### Added
- Initial Place Print source code
